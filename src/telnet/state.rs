...
/// "State machine" which holds and handles the current state of a telnet session.
pub struct State {
    /// Buffer of "readable" received data that can be obtained by using the `Read` trait
    output_buffer: Vec<u8>,
    /// Current overall mode
    mode: Mode,
    /// Returns whether every incoming, non-command char should be echoed back to the client
    is_echoing: bool,
    /// If true, ANSI escape sequences will be handled like normal non-command input.
    /// Otherwise, sequences will be ignored and the client receives a BEL to notice.
...
/// Enumeration of modes that a telnet session may have
enum Mode {
...
impl State {
    ...
    /// # Arguments
    ///
    /// * `config` - Configuration on e.g. how to handle certain input during the
    ///   session
    ///
    /// # Returns
    ///
    /// Fresh `State` for a telnet session
    pub fn new_with_config(config: &StateConfig) -> Self {
        State {
            output_buffer: vec![],
            mode: Mode::Idle,
            is_echoing: false,
    ...
    /// Creates new `State` with default configuration
    ///
    /// # Returns
    ///
    /// Fresh `State` for a telnet session
...
impl State {
    /// Writes a buffer of received telnet data into the session, returning an
